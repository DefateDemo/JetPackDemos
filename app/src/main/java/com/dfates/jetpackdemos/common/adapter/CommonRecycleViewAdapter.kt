 ...
 */
open class CommonRecycleViewAdapter<M>(mContext: Context, layoutIds: SparseIntArray?, mDatas: List<M?>?, private var convert: CommonRecycleViewAdapterConvert<M>?) : BaseRecycleViewAdapter<M>(mContext, layoutIds, mDatas) {
    /**
     * 使用arrayOf(viewType to layoutId,...) 的形式传入多个布局
     */
    constructor(mContext: Context, layoutIds: Array<Pair<Int, Int>>, mDatas: List<M?>?, convert: CommonRecycleViewAdapterConvert<M>?) : this(mContext, layoutIds.toSparseIntArray(), mDatas, convert)

    /**
     * 传入单个布局
     */
    constructor(mContext: Context, layoutId: Int, mDatas: List<M?>?, convert: CommonRecycleViewAdapterConvert<M>?) : this(mContext, arrayOf(0 to layoutId), mDatas, convert)
